...
class NinV2Controller extends Controller
{
     /**
     * Display the NIN Verification Documentation and Test Form.
     */
    public function index()
    {
        ...
        $field = ServiceField::where('field_code', 'V606')->first(); // Used 605 for NIN Phone
        if (!$field) {
            $field = ServiceField::create([
                'service_id' => $service->id,
                'field_name' => 'NIN v2 Verification',
                'field_code' => 'V606',
                'base_price' => 0.00,
                'is_active' => true,
            ]);
        ...
        return view('api.nin_v2', compact('user', 'price', 'field'));
    }

    /**
     * Process NIN V2 Verification Request.
     * 
    ...
    public function store(Request $request)
    {
        try {
            ...
            $field = ServiceField::where('field_code', 'V606')->first();
            
            if (!$field || !$field->is_active) {
                return response()->json([
                    'status' => false,
                    'message' => 'NIN V2 Verification service is currently unavailable.'
                ], 503);
            }
            ...
            $transactionRef = 'NINV2' . date('is') . strtoupper(Str::random(5));
            $performedBy = $user->first_name . ' ' . $user->last_name;

            // 6. External API Call
            $endpoint = "https://raudah.com.ng/api/nin/";
            ...
            try {
                ...
                return DB::transaction(function () use ($wallet, $price, $apiData, $user, $field, $transactionRef, $performedBy, $data, $request) {
                    ...
                    // Debit Wallet Securely (Atomic update)
                    $updated = Wallet::where('id', $wallet->id)
                        ->where('balance', '>=', $price)
                        ->decrement('balance', $price);

                    if (!$updated) {
                        throw new \Exception('Insufficient wallet balance during final deduction.');
                    }
                    ...
                    $transaction = Transaction::create([
                        ...
                        'user_id' => $user->id,
                        'amount' => $price,
                        'description' => "NIN V2 Verification for {$request->nin}",
                        'type' => 'debit',
                        'status' => 'completed',
                        'trans_source' => 'API',
                    ...
                    return response()->json([
                        'status' => true,
                        'message' => 'NIN V2 Verification Successful',
                        'api_response' => $data,
                    ], 200);
                });

            } catch (\Exception $e) {
                Log::error('NIN V2 API/Transaction Error: ' . $e->getMessage());
                throw $e;
            }

        } catch (\Throwable $e) {
            Log::error('NIN V2 Critical Error: ' . $e->getMessage());
